...
**Capability token** — минимальный набор прав, выдаваемый исполнителю на один конкретный вызов, вместо постоянных широких разрешений. Ограничивает ущерб при перехвате намерения.

**Cascade depth** — метрика распространения ошибки в multi-agent системе: сколько ролей приняли неверное upstream-заключение и передали дальше. Измеряется внедрением правдоподобного ложного свидетельства.
...
**Durable state** — состояние задачи, живущее вне процесса и переживающее рестарт: цель, решения, ограничения, прогресс, артефакты, незавершённая работа, план отката.
...
**Grounded Retrieval Layer** — слой извлечения, сочетающий плотный и разреженный поиск, контекстные префиксы фрагментов и provenance каждого результата.
...
**Memory poisoning** — попадание вредоносной инструкции в долговременную память через контент, который агент записывает сам. Отличается от разовой инъекции тем, что переживает сессию.

**Pass@k / Pass^k** — вероятность успеха хотя бы в одной из k попыток против вероятности успеха во всех k. Первая отвечает на вопрос о способности, вторая — о надёжности; подмена создаёт ложное впечатление стабильности.

**Poka-yoke** — проектирование интерфейса так, чтобы ошибка была невозможна по конструкции, а не запрещена инструкцией. Применительно к инструментам: схема, не допускающая неверного вызова.
