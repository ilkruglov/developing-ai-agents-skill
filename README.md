...
### v3: версия 0.5.0 против 0.4.0

26 сценариев, один прогон на конфигурацию, слепая парная оценка.

| Метрика | 0.5.0 | 0.4.0 |
|---|---:|---:|
| Выполненные критерии | 90,5% (95/105) | 91,4% (96/105) |
| Победы в слепом сравнении | 8 | 11 |
| Ничьих | 7 | |

**Улучшение версии 0.5.0 не доказано.** Точный тест McNemar по разошедшимся
критериям даёт p = 1,000, биномиальный тест побед — p = 0,648; наблюдаемая
разница в 1,0 процентного пункта меньше стандартной ошибки в 2,9 пункта.

Расширение справочных материалов дало проверяемость цитат и навигацию, но на
этом наборе не изменило качество ответов.

Прогон выявил регрессию в шести сценариях: механизмы, лежавшие в 0.4.0 прямо в
`SKILL.md`, были вынесены в конспекты глав, куда маршрут агента не ведёт. В
версии 0.5.1 они возвращены в ядро, и на повторном прогоне этих сценариев
версии сравнялись. Это устранение регрессии, а не доказательство улучшения:
правка вносилась со знанием проваленных критериев, поэтому набор перестал быть
независимым. Подробности и ограничения — в
[`benchmarks/v3/benchmark.md`](plugins/developing-ai-agents/benchmarks/v3/benchmark.md).
